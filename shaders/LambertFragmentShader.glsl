...
vec4 calcLight(Light light, vec3 lightDir, vec3 normal, vec4 fragTexColor) {
...
vec4 calcPointLight(Light light, vec3 normal, vec3 fragPos, vec4 fragTexColor) {
	vec3 lightDir = normalize(light.pos - fragPos);
	vec4 resultColor = calcLight(light, lightDir, normal, fragTexColor);

	float attenuation = calcAttenuation(light, fragPos);
	return resultColor * attenuation;
}

vec4 calcDirectionalLight(Light light, vec3 normal, vec4 fragTexColor) {
	//light dir is given as pointing away from the light source, so it's negated now to point away from the frag
	vec3 lightDir = normalize(-light.direction); 
	return calcLight(light, lightDir, normal, fragTexColor);
}

vec4 calcSpotlight(Light light, vec3 normal, vec3 fragPos, vec4 fragTexColor) {
	vec3 lightDir = normalize(light.pos - fragPos);
	...
	if(angleCosine > light.cutOff) {
		...
		vec4 diffuse = calcDiffuse(light, lightDir, normal, fragTexColor);
		vec4 resultColor = ambient + diffuse * intensity;
	...
	for(int i = 0; i < lightCount; i++) {
		if(lights[i].enabled) {
			switch (lights[i].type) {
				case POINT_LIGHT:
					resultColor += calcPointLight(lights[i], normal, outWorldPos, fragTexColor);
					break;
				case DIRECTIONAL_LIGHT:
					resultColor += calcDirectionalLight(lights[i], normal, fragTexColor);
					break;
				case SPOTLIGHT:
					resultColor += calcSpotlight(lights[i], normal, outWorldPos, fragTexColor);
					break;
			}
		}
	}
